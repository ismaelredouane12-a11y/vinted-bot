...
# ══════════════════════════════════════════════════════════════════════════════
TYPES_ARTICLES = {
    "tshirt":   ["t-shirt", "tshirt", "tee shirt", "tee-shirt", " tee ", "t shirt"],
    "hoodie":   ["hoodie", "sweat", "sweater", "sweatshirt", "crewneck", "crew neck", "pullover"],
    "veste":    ["veste", "jacket", "blouson", "bomber", "windbreaker", "coach jacket", "track jacket", "fleece"],
    "manteau":  ["manteau", "coat", "parka", "puffer", "doudoune", "trench"],
    "short":    ["short", "shorts"],
    "pantalon": ["pantalon", "cargo", "pant", "jogger", "jogging", "baggy", "jean", "denim", "chino", "trackpant", "track pant"],
    "ensemble": ["ensemble", "survêtement", "survete", "tracksuit", "set "],
    "polo":     ["polo"],
    "chemise":  ["chemise", "shirt", "flannel"],
    "debardeur":["débardeur", "debardeur", "tank top", "tank"],
    "chaussure":["chaussure", "sneaker", "basket", "shoe", "boot", "trainer", "running", "air max", "air force", "jordan", "dunk", "yeezy", "350", "990", "2002", "550"],
    "casquette":["casquette", "cap", "bonnet", "beanie", "hat"],
    "sac":      ["sac", "bag", "backpack", "tote"],
    "accessoire":["ceinture", "belt", "portefeuille", "wallet", "collier", "bracelet", "bague", "ring"],
}

def detecter_type_article(titre: str) -> str | None:
    """Détecte le type d'article à partir du titre (retourne une clé de TYPES_ARTICLES ou None)."""
    t = titre.lower()
    for type_key, mots_cles in TYPES_ARTICLES.items():
        if any(mc in t for mc in mots_cles):
            return type_key
    return None

def types_compatibles(type_a: str | None, type_b: str | None) -> bool:
    """Vérifie que deux types d'articles sont compatibles pour la comparaison."""
    if type_a is None or type_b is None:
        # Si on ne sait pas détecter, on laisse passer (pas de faux négatif)
        return True
...
def _fetch_similaires_sync(marque: str, category_id: int | None, taille: str,
                            type_article: str | None = None, per_page: int = 20) -> list:
    """Recherche des articles similaires par marque + type d'article."""
    global _session
    # On inclut le type dans la requête pour guider l'API
    if type_article and type_article in TYPES_ARTICLES:
        mot_type = TYPES_ARTICLES[type_article][0]  # premier mot-clé = le plus générique
        query = f"{marque} {mot_type}"
    else:
        query = marque
    url = (
        f"https://www.vinted.fr/api/v2/catalog/items"
...
def _trouver_similaires_sync(marque: str, category_id: int | None,
                              ...
                              type_article: str | None = None,
                              exclure_id: int | None = None) -> list[dict]:
    """
    Cherche 3-10 articles similaires et retourne leurs infos de prix.
    Filtre par type d'article (t-shirt avec t-shirts, hoodie avec hoodies, etc.)
    et par taille si possible.
    """
    raw = _fetch_similaires_sync(marque, category_id, taille, type_article, per_page=30)
    resultats = []
    for item in raw:
        iid  = item.get("id")
        if iid == exclure_id:
        ...
        if prix is None or prix <= 0:
            continue
        # Plage de prix resserrée : ±50% autour du prix de référence (était ×5)
        if prix < prix_ref * 0.5 or prix > prix_ref * 2.5:
            continue

        titre_sim = item.get("title", "") or ""

        # ── Filtre type d'article ──────────────────────────────────────────
        type_sim = detecter_type_article(titre_sim)
        if not types_compatibles(type_article, type_sim):
            continue   # On rejette les articles d'un type différent
...
def analyse_propagation_sync(item: dict, profondeur: int = 0,
    ...
    # ── Détection du type d'article ──────────────────────────────────────
    type_article = detecter_type_article(titre)

    category_id = item.get("catalog_id") or item.get("category_id")

    # 1. Chercher similaires du MÊME TYPE
    similaires = _trouver_similaires_sync(
        marque, category_id, taille, prix,
        type_article=type_article, exclure_id=item_id
    )
    ...
    if ok:
        ...
        data["type_article"] = type_article or "?"
        if similaires:
    ...
    if profondeur < 2 and similaires:
        for sim in similaires[:3]:
            ...
            if sim_prix < prix * 0.85 and sim["id"] not in visites:
                ...
                if sim_item:
                    # Vérifie que l'item propagé est du même type avant d'explorer
                    type_sim_item = detecter_type_article(sim_item.get("title", "") or "")
                    if types_compatibles(type_article, type_sim_item):
...
def formater_caption(d: dict) -> str:
    ...
    type_txt = d.get("type_article", "")
    sim_txt  = f"({nb_sim} {type_txt}s comparés)" if nb_sim and type_txt and type_txt != "?" else (f"({nb_sim} similaires)" if nb_sim else "")
    return (
